package reisaks.Bootcamp2023.async
...
object Exercise1 extends App {
  def firstCompleted[T](f1: Future[T], f2: Future[T])(implicit ec: ExecutionContext): Future[T] = {
    ...
    // Attach onComplete callbacks to both futures
    f1.onComplete(prom.tryComplete)
    f2.onComplete(prom.tryComplete)
...
object Exercise2 extends App {
  def sumAll(futureValues: Seq[Future[Int]])(implicit ec: ExecutionContext): Future[Int] =
    futureValues.foldLeft(Future.successful(0)) { (acc, w) =>
      for {
        accumulated <- acc
        currentFuture <- w
      } yield accumulated + currentFuture
    }
