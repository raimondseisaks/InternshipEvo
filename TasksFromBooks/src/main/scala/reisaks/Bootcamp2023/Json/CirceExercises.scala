package reisaks.Bootcamp2023.Json
...
object CirceExercises {
  ...
  object basics {
    ...
    lazy val jMatrix: Json = Json.obj(
      "title" -> Json.fromString("The Matrix"),   // or "The Matrix".asJson
  ...
  object optics {
    ...
    val excersise3 = root.artist.ontour.boolean.modify(_ => true)
    lazy val killersOnTourJson: Json = excersise3(parsedKillersJson)
  }
  ...
  object semiauto {
    ...
    val albumJson: Json = Album("Lit", 1992, Seq(song, song1)).asJson
  }
  ...
  object manual {
    ...
    /*implicit val songDecoder2: Decoder[Song] =       //manually define fields
    ...
    val albumJson: Json = Album("Lit", 1992, Seq(song)).asJson
  }
  ...
  object custom1 {
   ...
   // val timeWindowJson: Json   = timeWindow.asJson
    ...
    implicit lazy val encodeYear: Encoder[Year] = Encoder.encodeInt.contramap(_.getValue)
    implicit lazy val decodeYear: Decoder[Year] = Decoder.decodeInt.emap {
      int =>
        Either
          .catchNonFatal(Year.of(int))
          .leftMap(err => "Year: " + err.getMessage)
    }
  }
  ...
  object snake_case {
    ...
    implicit val config: Configuration = Configuration.default.withDiscriminator("artist")
