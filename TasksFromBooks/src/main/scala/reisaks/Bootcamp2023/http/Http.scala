package reisaks.Bootcamp2023.http
...
import reisaks.Bootcamp2023.http.Protocol._
import org.http4s._
...
object HttpServer extends IOApp {
  ...
  private val paramsRoutes = {
    ...
    HttpRoutes.of[IO] {
      ...
      case GET -> Root / "validate" :? TimestampMatcher(timestamp) =>
        if (timestamp.isValid)
          Ok("Timestamp is valid")
        else
          BadRequest("Timestamp is invalid")
    }
  }
    ...
    case req @ POST -> Root / "multipart" =>
      req.as[Multipart[IO]].flatMap { multipart =>
        val res = for {
          ...
          c <- OptionT(multipart.parts.find(_.name.contains("character"))
          .traverse(_.as[String]))
            .subflatMap { s =>
              Option.when(s.length ==1)(s.head)
            }
          count <- OptionT(
            multipart.parts
        ...
        res.cataF(BadRequest(), count => Ok(count.toString))
    ...
    EmberClientBuilder
      ...
      .use { client =>
        for {
          ...
          // curl "localhost:9001/params/validate?timestamp=2020-11-04T14:19:54.736Z"
          _ <- client.expect[String]((uri / "validate").withQueryParam("timestamp", "2020-11-04T14:19:54.736Z"))
          _ <- printLine()
