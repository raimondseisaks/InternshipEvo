...
object RefinedScala {
  ...
  //
  case class Document(url: Refined[String, Url], body: Refined[String, Xml])
...
object ValueClasses {
  ...
  def transferMoney(from: fromAccount, to: toAccount, amount: BigDecimal): Unit = {
    // some logic
...
object ImpossibleState {
  ...
  case class Finished(id: String, finishedAt: Long) extends Task
  case class Running(id: String, progress: Double) extends Task
  case class Cancelled(id: String) extends Task
...
object Parametricity {
  ...
  def f5_way2[A](as: List[A]): List[A] = as.reverse
  def f5_way3[A](as: List[A]): List[A] = as.empty

  // How many ways we can implement this function with?
  def f6[A, B](as: List[A]): List[B] = List.empty
...
class EffectTrackingSpec extends AnyFunSuite {
  ...
  class FakePrinting extends Printing {
  ...
  test("Service.call prints out anything") {
    val printing: FakePrinting = new FakePrinting()
    val service: Service = new Service(printing, () => 0L)
    service.call("Hello")
    println(printing.printed)
  }

  test("Service.call prints out correct message with current time") {
    val printing: FakePrinting = new FakePrinting()
    val time = System.nanoTime()
    val service: Service = new Service(printing, () => time)
    service.call("Hello")
    assert(printing.printed == List(s"$time Hello"))
  }
